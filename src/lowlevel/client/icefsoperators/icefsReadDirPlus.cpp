...
void IcefsClient::DoIcefsReadDirPlus(fuse_req_t fuseReq, fuse_ino_t inode,
  ...
  switch (this->clientConfig.linkType) {
    case ICEFS_LINK_USE_GRPC: {
      icefsgrpc::IcefsReadDirPlusReq req;
      icefsgrpc::IcefsReadDirPlusRes res;
      ...
      grpc::Status status = gRpcClient->DoIcefsReadDirPlus(&ctx, req, &res);
      if (status.ok() && !res.status()) {
        ...
        for (int i = 0; i < dataSize; ++i) {
          IcefsGRpcFillFuseEntryParamIn(&entry, res.data(i).entry());
          entrySize = fuse_add_direntry_plus(
              fuseReq, p, remain, res.data(i).dir_entry().name().c_str(),
    ...
    case ICEFS_LINK_USE_THRIFT: {
      icefsthrift::IcefsReadDirPlusReq req;
      icefsthrift::IcefsReadDirPlusRes res;
      ...
      thriftConn->thriftClient->DoIcefsReadDirPlus(res, req);
      ...
      if (!res.status) {
        ...
        struct fuse_entry_param entry = {0};
        for (int i = 0; i < dataSize; ++i) {
          IcefsThriftFillFuseEntryParamIn(&entry, res.data[i].entry);
