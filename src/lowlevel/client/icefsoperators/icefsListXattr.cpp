...
void IcefsClient::DoIcefsListXattr(fuse_req_t fuseReq, fuse_ino_t inode,
  ...
  switch (this->clientConfig.linkType) {
    case ICEFS_LINK_USE_GRPC: {
      icefsgrpc::IcefsListXattrReq req;
      icefsgrpc::IcefsListXattrRes res;
      grpc::ClientContext ctx;

      req.set_inode(inode);
      req.set_size(size);

      grpc::Status status = gRpcClient->DoIcefsListXattr(&ctx, req, &res);
    ...
    case ICEFS_LINK_USE_THRIFT: {
      icefsthrift::IcefsListXattrReq req;
      icefsthrift::IcefsListXattrRes res;

      req.__set_inode(inode);
      req.__set_size(size);

      icefsThriftConn *thriftConn = thriftClientPool->GetIcefsThriftConn();
      thriftConn->thriftClient->DoIcefsListXattr(res, req);
