...
void IcefsClient::DoIcefsFlock(fuse_req_t fuseReq, fuse_ino_t inode,
  ...
  switch (this->clientConfig.linkType) {
    case ICEFS_LINK_USE_GRPC: {
      icefsgrpc::IcefsFlockReq req;
      icefsgrpc::IcefsFlockRes res;
      grpc::ClientContext ctx;

      req.set_fh(fi->fh);
      req.set_op(op);

      grpc::Status status = gRpcClient->DoIcefsFlock(&ctx, req, &res);
    ...
    case ICEFS_LINK_USE_THRIFT: {
      icefsthrift::IcefsFlockReq req;
      icefsthrift::IcefsFlockRes res;

      req.__set_fh(fi->fh);
      req.__set_op(op);

      icefsThriftConn *thriftConn = thriftClientPool->GetIcefsThriftConn();
      thriftConn->thriftClient->DoIcefsFlock(res, req);
